        ...
        <aside className="sidebar">
          ...
          <nav className="sidebar-nav">
            <div className="nav-group">
              ...
              <div className={`nav-item ${filter === "payment" ? "active" : ""}`} onClick={() => setFilter("payment")}>
                <span className="nav-icon">💳</span> Payment Cards
              </div>
              <div className={`nav-item ${filter === "id" ? "active" : ""}`} onClick={() => setFilter("id")}>
                <span className="nav-icon">🪪</span> ID Cards
              </div>
              <div className={`nav-item ${filter === "license" ? "active" : ""}`} onClick={() => setFilter("license")}>
                <span className="nav-icon">🚗</span> Drivers License
              </div>
              <div className={`nav-item ${filter === "insurance" ? "active" : ""}`} onClick={() => setFilter("insurance")}>
                <span className="nav-icon">🛡️</span> Insurance
              </div>
              <div className={`nav-item ${filter === "documents" ? "active" : ""}`} onClick={() => setFilter("documents")}>
          ...
          <header className="main-header">
            <div className="header-search">
              <div className="search-box">
                ...
                <input type="text" placeholder="Search cards, documents..." />
    ...
    return (
      <main className="onboarding-container">
        ...
        <style jsx>{`
          ...
          .btn-primary { background: #2563eb; color: white; border: none; padding: 14px; border-radius: 12px; font-weight: 600; cursor: pointer; }
  ...
  if (step === "generate") {
    return (
      <main className="onboarding-container">
        ...
        <style jsx>{`
          .onboarding-container { display: flex; align-items: center; justify-content: center; min-height: 100vh; padding: 20px; background: radial-gradient(circle at bottom left, #1e1b4b, #0f172a); }
          .onboarding-card { max-width: 600px; background: rgba(255, 255, 255, 0.03); backdrop-filter: blur(20px); padding: 40px; border-radius: 32px; border: 1px solid rgba(255, 255, 255, 0.1); }
          h2 { color: white; font-weight: 800; margin-bottom: 8px; }
          .warning-text { color: #fca5a5; margin-bottom: 32px; font-size: 14px; }
          .mnemonic-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; margin-bottom: 32px; }
          .mnemonic-word { background: rgba(255, 255, 255, 0.05); padding: 12px; border-radius: 8px; color: white; font-family: monospace; display: flex; gap: 10px; }
          .mnemonic-index { color: #64748b; }
          .btn-primary { background: #2563eb; color: white; border: none; padding: 16px; border-radius: 12px; font-weight: 600; cursor: pointer; }
        `}</style>
      </main>
    );
